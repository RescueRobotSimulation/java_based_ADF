...
public class TIMRADKmeansPP_ extends StaticClustering {
    ...
    public TIMRADKmeansPP_(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        super(ai, wi, si, moduleManager, developData);
        this.repeatPrecompute = developData.getInteger("sample.module.SampleKMeans.repeatPrecompute", 7);
        //this.repeatPreparate = developData.getInteger("sample.module.SampleKMeans.repeatPreparate", 10);
        this.repeatPreparate = 10;
        this.clusterSize = developData.getInteger("sample.module.SampleKMeans.clusterSize", this.calcClusterSize());
    ...
    private int calcClusterSize(){
        int size = worldInfo.getEntitiesOfType(agentInfo.me().getStandardURN()).size()/4;
    ...
    public Clustering resume(PrecomputeData precomputeData) {
        ...
        this.centerList = new ArrayList<>();
        for (EntityID id : centerIDs){
            centerList.add(this.worldInfo.getEntity(id));
        }
    ...
    public Collection<StandardEntity> getClusterEntities(int index) {
        ...
        if (index < 0 || index >= this.clusterEntityIDsList.size()) {
    ...
    private void assign() {
        ...
        if (this.assignAgentsFlag) {
            ...
            List<StandardEntity> ambulanceteamList = new ArrayList<>(this.worldInfo.getEntitiesOfType(AMBULANCE_TEAM));
    ...
    private void calcStandard(int repeat) {
        ...
        List<StandardEntity> entityList;
        if (this.entities == null || this.entities.size() <= 0) {
            return;
        }

        entityList = new ArrayList<>(this.entities);
    ...
    private class IdSorter implements Comparator<StandardEntity> {
