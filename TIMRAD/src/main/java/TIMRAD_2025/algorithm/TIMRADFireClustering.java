...
public class TIMRADFireClustering extends DynamicClustering {
	private int groupingDistance;
	private List<List<StandardEntity>> clusterList = new LinkedList<>();
	private List<EntityID> changedList;

	public TIMRADFireClustering(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
			DevelopData developData) {
		super(ai, wi, si, moduleManager, developData);
		this.groupingDistance = developData
				.getInteger("adf.core.component.module.algorithm.SampleFireClustering.groupingDistance", 30);
	...
	public Clustering calc() {
		for (EntityID changed :changedList) {
			StandardEntity changedEntity = worldInfo.getEntity(changed);
			if (changedEntity.getStandardURN().equals(StandardEntityURN.BUILDING)) { // changedEntity(cE)											// is
																						// a
																						// building
				Building changedBuilding = (Building) changedEntity;
				if (this.getClusterIndex(changedEntity) < 0) { // cE is not
																// contained in
																// cluster
					if (isBurning(changedBuilding)) { // cE is burning building
						...
						if (hostClusterPropertyEntityIDs.size() == 0) { // there
																		// is
																		// not
																		// host
																		// cluster
						...
						} else if (hostClusterPropertyEntityIDs.size() == 1) { // there
																				// is
																				// one
																				// host
																				// cluster
						...
						} else { // there are multiple host clusters : add
									// building to the cluster & combine
									// clusters
							int hostIndex = this.getClusterIndex(hostClusterPropertyEntityIDs.get(0));
					...
					if (!(isBurning(changedBuilding))) { // cE is not burning
															// building
	...
	public Clustering updateInfo(MessageManager messageManager) {
		...
		for (CommunicationMessage message : messageManager.getReceivedMessageList(CommandFire.class)) {
			...
			if (command.getAction() == CommandFire.ACTION_EXTINGUISH && command.isBroadcast()) {
				changedList.add(command.getTargetID());
			}
		}
		for (CommunicationMessage message : messageManager.getReceivedMessageList(MessageBuilding.class)) {
			...
			if (building.isOnFire()&&!changedList.contains(building.getID())) {
				this.changedList.add(building.getID());
			} else if(!building.isOnFire()){
				this.changedList.remove(mb.getBuildingID());
