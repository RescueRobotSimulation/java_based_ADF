	...
	private class Node {
		...
		public Node(Node parent, EntityID id) {
			...
			if(parent == null){
				...
				this.cost = parent.getCost() + worldInfo.getDistance(parent.getID(), id) + getClearCost(id) + getFireCost(id);
			}
		 ...
		 * @author sakaue(15th)
		 */
		private double getFireCost(EntityID id){
			StandardEntity standardEntity = worldInfo.getEntity(id);
			if(standardEntity instanceof Building){
				Building building = (Building)standardEntity;
				if(building.isOnFire()){
					double cost = building.getTemperature();
					return cost;
				}else if(building.getNeighbours() != null && !building.getNeighbours().isEmpty()) {
					double cost = 0;
					int neighbourSize = 0;
					for(EntityID neighbourID : building.getNeighbours()) {
						if(worldInfo.getEntity(neighbourID) instanceof Building) {
							Building neighbour = (Building)worldInfo.getEntity(neighbourID);
							if(neighbour.isTemperatureDefined()) {
								cost += ((Building)neighbour).getTemperature();
								neighbourSize++;
							}
						}
					}
					if(neighbourSize > 0) {
						return cost /= neighbourSize;
					}
					return 0;
				}
			}
