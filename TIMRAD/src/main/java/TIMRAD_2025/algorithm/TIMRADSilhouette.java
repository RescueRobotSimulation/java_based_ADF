...
public class TIMRADSilhouette extends StaticClustering {
	...
	public TIMRADSilhouette(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
		...
		this.clusterSize = this.calcClusterSize(); // クラスター数の決定
	}

	private int calcClusterSize(){
		int size = 1;

		double silhouetteValue; //シルエット値
		double maxValue = -1; // 最大のシルエット値

		int maxClusterSize = (int)Math.sqrt(this.worldInfo.getEntitiesOfType(myurn).size());
		if(maxClusterSize < 1) {
			maxClusterSize = 1;
		}

		for(int i = 1; i <= maxClusterSize; i++) {
			this.execute(REPEATS_PRECOMPUTE, i); // クラスタリング
			silhouetteValue = this.getSilhouetteValue(this.clusters); // シルエット値の導出
			this.clusters.clear();
			if(silhouetteValue > maxValue) {
				maxValue = silhouetteValue;
				size = i;
			}
		}
	...
	private double getSilhouetteValue(ArrayList<Cluster> clusters) {
		double s = 0; // シルエット値
		double a; // 凝集度
		double b; // 乖離度

		Cluster neighboringCluster; // 隣接するクラスター
		double minDistance; // 最も近いのクラスターとの距離
		double distance; // 他のクラスターとの距離

		for(Cluster cluster : clusters) {

			// 隣接するクラスターの検出
			neighboringCluster = null;
			minDistance = Integer.MAX_VALUE;
			for(Cluster otherCluster : clusters) {
				if(otherCluster.equals(cluster)) {
					continue;
				}
				distance = Math.hypot(cluster.centerX - otherCluster.centerX, cluster.centerY - otherCluster.centerY);
				if(distance < minDistance) {
					minDistance = distance;
					neighboringCluster = otherCluster;
				}
			}

			for(StandardEntity entity : cluster.elements) {
				int entityX = this.worldInfo.getLocation(entity).first();
				int entityY = this.worldInfo.getLocation(entity).second();

				// 凝集度の計算
				a = Math.hypot(entityX - cluster.centerX, entityY - cluster.centerY);

				// 乖離度の計算
				if(neighboringCluster != null) {
					b = Math.hypot(entityX - neighboringCluster.centerX, entityY - neighboringCluster.centerY);
				}else {
					b = 0;
				}
	...
	public Clustering precompute(PrecomputeData precomputeData) {
		...
		this.execute(REPEATS_PRECOMPUTE, this.clusterSize);
	...
	public Clustering preparate() {
		...
		this.execute(REPEATS_PREPARE, this.clusterSize);
	...
	public void execute(int repeat, int clusterSize) {
		this.clusters = getInitialCenter(clusterSize);
	...
	private ArrayList<Cluster> getInitialCenter(int clusterSize) {
