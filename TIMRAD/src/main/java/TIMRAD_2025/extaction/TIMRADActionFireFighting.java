...
public class TIMRADActionFireFighting extends ExtAction {
    ...
    public TIMRADActionFireFighting(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo, ModuleManager moduleManager, DevelopData developData) {
    ...
    private void reset() {
        ...
        if (clustering != null) {
            ...
            clusterEntities.addAll(this.clustering.getClusterEntities(clustering.getClusterIndex(agentInfo.getID())));
        }
    ...
    private Action calcRefill() {
        ...
        } else { /*メモ:これがあるからisRefugeがtrueになるだけでrefugeに移動できる*/
            //リフジに移動
            List<EntityID> path = null;
            pathPlanning.setFrom(fireBrigade.getPosition());
