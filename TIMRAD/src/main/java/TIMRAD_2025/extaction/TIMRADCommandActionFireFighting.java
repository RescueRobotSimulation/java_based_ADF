...
public class TIMRADCommandActionFireFighting extends ExtAction
{
	...
	public TIMRADCommandActionFireFighting(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo, ModuleManager moduleManager, DevelopData developData)
	{
		...
		case PRECOMPUTATION_PHASE:
			this.pathPlanning = moduleManager.getModule("ActionFireFighting.PathPlanning", "RIO2023.algorithm.AstarPathPlanning");
			this.clustering = moduleManager.getModule("ActionFireFighting.Clustering.Fire", "RIO2023.algorithm.RioneKmeansPP");
			break;
		case PRECOMPUTED:
			this.pathPlanning = moduleManager.getModule("ActionFireFighting.PathPlanning", "RIO2023.algorithm.AstarPathPlanning");
			this.clustering = moduleManager.getModule("ActionFireFighting.Clustering.Fire", "RIO2023.algorithm.RioneKmeansPP");
	...
	public ExtAction calc(){
		...
		if(this.target == null){
			//System.out.println("再探索開始");
			//this.result = calcSearch();//targetがない＝探索が必要
			return this;
		}
	...
	private Action calcInBlockade(FireBrigade agent){
	...
	private Action calcExtinguish(FireBrigade agent, PathPlanning pathPlanning, EntityID target){
		...
		if(StandardEntityURN.REFUGE == positionEntity.getStandardURN()){     ///エージェントがREFUGEにいるとき近くの燃えてる建物消火

			/////秋キャンプ ↓///////////////////////////////////////////////////////agentの水の残量が９割に満たないとき給水、９割以上で移動
			int Maxwater = scenarioInfo.getFireTankMaximum();
		...
		/////REFUGEにいないときtargetが消火可能範囲なら消火、無理なら近くまで移動
		List<StandardEntity> neighbourBuilding = new ArrayList<>();
		StandardEntity entity = this.worldInfo.getEntity(target);
		if(entity instanceof Building){
			if(calculateWallDistance(entity) < maxExtinguishDistance){
		...
		/*List<StandardEntity> FBs = new ArrayList<>();
		for(EntityID en:this.worldInfo.getChanged().getChangedEntities()){
			StandardEntity st = this.worldInfo.getEntity(en);
			if(st instanceof FireBrigade) {
				FBs.add(st); //視界内のFBを数える
			}
			}*/
		...
		for(EntityID en:this.worldInfo.getChanged().getChangedEntities()){
			//if(FBs.size()<5) {
			StandardEntity st = this.worldInfo.getEntity(en);
			if(st instanceof Building && ((Building)st).isOnFire()) {
				neighbourBuilding.add(st); //視界内の燃えてる建物を消火候補に追加
				//}
			}
		}
		if(neighbourBuilding.size() > 0){
	...
	private Action getMoveAction(PathPlanning pathPlanning, EntityID from, EntityID target){
		...
		return null;
		/*path = recalclatePath(pathPlanning,from,target);//targetを再計算し，pathも再計算
		if(path != null) {
	...
	//nishida
	private List<EntityID> recalclatePath(PathPlanning pathPlanning, EntityID from, EntityID preTarget){
		this.target = reCalclateTarget(preTarget);
		if(target==null)
			return null;
		pathPlanning.setFrom(from);
		pathPlanning.setDestination(target);
	...
	private Action calcRefill(){
		...
		this.thresholdCompleted = (maxWater / 10) * 10;
		this.thresholdRefill = maxExtinguishPower * 1;
	...
	private Action calcEfficientRefill(FireBrigade fireBrigade, PathPlanning pathPlanning, EntityID target,StandardEntity hydrant){
		...
		{
			if(hydrantPath!=null&&!hydrantPath.isEmpty()) {
				//System.out.println(fireBrigade.getID()+":hyd");
				isRefill = true;
				return getMoveAction(pathPlanning, fireBrigade.getPosition(), hydrant.getID());
			}
		}else
			...
			if(visibleFireBrigades.size() > 3
				...
				if(!fireBuildings.isEmpty()){
					...
					if(number==0 && !(topNeighbor2.isEmpty())){
						...
						//System.out.println(agentInfo.me().getID().getValue() + "Comm");
	...
	private Action samplealcRefill(FireBrigade agent, PathPlanning pathPlanning, EntityID target)
	{
		StandardEntityURN positionURN = Objects.requireNonNull(this.worldInfo.getPosition(agent)).getStandardURN();
