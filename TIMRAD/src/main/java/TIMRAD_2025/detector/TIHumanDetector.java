...
import TIMRAD_2025.extaction.TIMRADHistoryManager;

import static rescuecore2.standard.entities.StandardEntityURN.*;


public class TIHumanDetector extends HumanDetector {
	...
	public TIHumanDetector(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
		...
		for (StandardEntity next : this.worldInfo.getEntitiesOfType(AMBULANCE_TEAM, FIRE_BRIGADE, POLICE_FORCE)) {
			...
			if (positionEntity != null && elements.contains(positionEntity) || elements.contains(h)) {
				if (h.isHPDefined() && h.isBuriednessDefined() && h.getHP() > 0 && h.getBuriedness() > 0) {
					if (h.getHP() < TIHumanDetector.HUMAN_MAX_HP / 2) {
