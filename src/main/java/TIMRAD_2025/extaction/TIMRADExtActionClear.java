...
import TIMRAD_2025.util.ambulancehelper.TIMRADSelectorTargetByDis;
import TIMRAD_2025.helptool.TIMRADWorldHelper;
import TIMRAD_2025.debugger.*;
...
public class TIMRADExtActionClear extends ExtAction {
    ...
    public TIMRADExtActionClear(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
    ...
    private Action moveOnPath(List<EntityID> path) {
        ...
        if (stuckFlag) {
            ...
            if (TIMRADConstants.DEBUG_STUCK_HELPER && action == null) {
    ...
    public boolean isStuck(List<EntityID> path) {
        if (lastStuckUpdateTime == agentInfo.getTime()) {
            ...
            int moveDistance = TIMRADSelectorTargetByDis.getDistance.distance(position, lastPosition);
