...
public class GraphHelper extends AbstractModule {
    ...
    public GraphHelper(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        ...
        areas = new HashSet<>(TIMRADWorldHelper.getAreasWithURN(wi));
        roads = new HashSet<>(TIMRADWorldHelper.getRoadsWithURN(wi));
        buildings = new HashSet<>(TIMRADWorldHelper.getBuildingsWithURN(wi));
    ...
    private boolean isOnTooSmallEdge(Area area, Node node) {
        ...
        for (Edge edge : area.getEdges()) {
            if (edge.isPassable()) {
                ...
                if (Ruler.getDistance(edge.getStartX(), edge.getStartY(), edge.getEndX(), edge.getEndY()) < TIMRADConstants.TOO_SMALL_EDGE_THRESHOLD
