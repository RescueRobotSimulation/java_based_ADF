...
public class TIMRADWall {
	...
	public TIMRADWall(int x1, int y1, int x2, int y2, TIMRADBuilding owner) {
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x2;
		this.y2 = y2;
	...
	public void findHits(TIMRADBuilding building) {
		this.selfHits = 0; 
		this.strange = 0;  
		for (int emitted = 0; emitted < this.rays; emitted++) {
			...
			TIMRADWall closest = null;
			double minDistance = Double.MAX_VALUE;
			for (TIMRADWall other : building.getAllWalls()) {
				if (other == this)
					continue;
            ...
            if (closest != this && closest != null && closest.owner != owner) {
                hits++;
                Hashtable<TIMRADBuilding, Integer> hashtable = building.getConnectedBuildingTable();
