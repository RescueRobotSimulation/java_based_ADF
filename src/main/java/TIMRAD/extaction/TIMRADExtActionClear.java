...
public class TIMRADExtActionClear extends ExtAction {
    ...
    private static final double MOVEMENT_THRESHOLD = 50.0;
    ...
    public TIMRADExtActionClear(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        ...
        this.lastX = agentInfo.getX();
        this.lastY = agentInfo.getY();
        this.positionHistory = new ArrayList<>();
        this.positionHistory.add(new Point2D(lastX, lastY));
        this.lastTime = agentInfo.getTime();
  ...
  public ExtAction precompute(PrecomputeData precomputeData) {
    ...
    precomputeData.setDouble("lastX", this.lastX);
    precomputeData.setDouble("lastY", this.lastY);
    precomputeData.setInteger("lastTime", this.lastTime);
  ...
  public ExtAction resume(PrecomputeData precomputeData) {
    ...
    this.lastX = precomputeData.getDouble("lastX");
    this.lastY = precomputeData.getDouble("lastY");
    this.lastTime = precomputeData.getInteger("lastTime");
  ...
  private Action clearNearestBlockade() {
        Human police = (Human) agentInfo.me();
        ...
        Point2D agentPosition = new Point2D(policeX, policeY);
        ...
        for (EntityID blockadeId : road.getBlockades()) {
            ...
            if (blockade != null && blockade.isApexesDefined()) {
                List<Line2D> edges = GeometryTools2D.pointsToLines(GeometryTools2D.vertexArrayToPoints(blockade.getApexes()), true);
                for (Line2D edge : edges) {
                    Point2D closestPoint = GeometryTools2D.getClosestPointOnSegment(edge, agentPosition);
                    double distance = calculateDistance(policeX, policeY, closestPoint.getX(), closestPoint.getY());
    ...
    private Action moveToRandomNeighbor(Road currentRoad) {
        List<EntityID> neighbours = currentRoad.getNeighbours();
    ...
    private Action clearBlockade(Blockade blockade) {
    ...
    private void updatePositionHistory() {
        ...
        if (currentTime > lastTime) {
            Point2D currentPosition = new Point2D(currentX, currentY);
        ...
        if (position instanceof Road || position instanceof Hydrant) {
            ...
            if (road.isBlockadesDefined()) {
                for (EntityID blockadeId : road.getBlockades()) {
                    Blockade blockade = (Blockade) worldInfo.getEntity(blockadeId);
                    if (blockade != null && blockade.isApexesDefined()) {
                        java.awt.Shape shape = blockade.getShape();
                        if (shape != null) {
                            if (shape.contains(policeX, policeY)) {
                                return blockade;
                            }
                        }
                    } 
    ...
    private boolean isNotMoving() {
        ...
        for (int i = 1; i < positionHistory.size(); i++) {
            Point2D prev = positionHistory.get(i - 1);
            Point2D curr = positionHistory.get(i);
            totalDisplacement += calculateDistance(prev.getX(), prev.getY(), curr.getX(), curr.getY());
        }
        double avgDisplacement = totalDisplacement / (positionHistory.size() - 1);
